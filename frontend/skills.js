...
/* `match_rate` — доля команд группы, которые после принятия пойдут в этот навык
   мимо Gemini: бэктест маршрутизирует каждую команду ровно так, как это сделает
   `/api/chat` (`backend/miner/backtest.py`, JEB-1562). Два соседних числа в
   карточку не выводятся: `generalization` (что получит ещё не написанная
   формулировка) и `agreement` (совпадение с планами учителя — это разброс сырья,
   а не качество навыка, и на карточке «принять навык?» читалось бы как оценка
   навыка). Подпись здесь и смысл числа там меняются вместе. */
